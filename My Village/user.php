...
$artikel = query("SELECT * FROM user");
?>
		...
		<div class="badan">
            ...
            <!-- <?php echo substr("hello", 0, 4) ?> -->
              ...
              <tr>
                <td><?= $i;  ?></td>
                <td>
                  <a href="ubah_user.php?ID=<?= $row["id"];?>"><i class="fa fa-pencil fa-fw">Ubah</i></a> |
                ...
                <td><?= substr($row["nama"],0, 5);?></td>
                <td><?= substr($row["email"],0, 10);  ?></td>
                <td><?= substr($row["password"],0, 40);  ?></td>
							</tr>
              <?php $i++; ?>
            <?php endforeach; ?>
            </table>
